...
document.addEventListener("DOMContentLoaded", function () {
  ...
  tableTallas = $("#table-tallas").dataTable({
    aProcessing: true,
    aServerSide: true,
    language: {
...
// --- CARGAR SELECT USO --- //
///////////////////////////////
let comboxUso = document.querySelector("#comboxUso");

//Cargo Todos los paises que tengo en la BD
function cargarUso() {
  $.ajax({
    type: "GET",
    url: base_url + "Tallas/mostrarUso",
    success: function (response) {
      //departamentos:Tengo el resultado en objeto
      const Uso = JSON.parse(response);

      let template =
        '<option class="form-control" selected disabled>-- Seleccione --</option>';

      Uso.forEach((tipo) => {
        template += `<option class="form-control" value="${tipo.cod_usos}">${tipo.description}</option>`;
      });

      comboxUso.innerHTML = template;
    },
  });
}
...
//*** GUARDAR NUEVO TALLA ***//
let formTalla = document.querySelector("#formTallas");

formTalla.addEventListener("submit", function (e) {
  e.preventDefault();

  let intIdTalla = document.querySelector("#idTallas").value; //Lo obtengo a la hora que voy a Editar
  let utilizar = document.querySelector("#comboxUso").value;
  let descripcion = document.querySelector("#txtDescripcion").value;
  let listStatus = document.querySelector("#listStatus").value;

  if (utilizar == "" || descripcion == "" || listStatus == "") {
    ...
    Swal.fire({
      ...
      icon: "warning",
      title: "Error!",
      text: "Los campos utilizar, descripcion, descripcion y estado no puede esta vacio",
      icon: "warning",
  ...
  let ajaxUrl = base_url + "Tallas/setTallas";
  let formDta = new FormData(formTalla);
  ...
  request.onload = function () {
    if (request.status == 200) {
      ...
      if (objData.status) {
        ...
        $("#table-tallas").DataTable().ajax.reload();
...
//*** ELIMINAR TALLA ***//
function fntDelTallas(idtalla) {
  Swal.fire({
    title: "Eliminar Talla",
    text: "¿Realmente quiere eliminar la talla?",
    icon: "warning",
    ...
    if (result.isConfirmed) {
      let request = new XMLHttpRequest();
      let ajaxUrl = base_url + "Tallas/delTallas";
      let strData = "cod_talla=" + idtalla;
...
function openModal() {
  document.querySelector("#idTallas").value = "";
  document
    .querySelector(".modal-header")
    .classList.replace("bg-pattern-2", "bg-pattern");
  document.querySelector("#titleModal").innerHTML = "Nueva talla";
  document
    .querySelector(".modal-header")
  ...
  document.querySelector("#formTallas").reset();
